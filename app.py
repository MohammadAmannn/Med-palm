...
with st.sidebar:
    selected = option_menu(
        menu_title=None,
        options=("Home", "About Developers", "Contact Us"),
...
if submit_button:
    if uploaded_file is not None:
        ...
        image_part = {"mime_type": f"image/{uploaded_file.name.split('.')[-1]}", "data": image_data}
        ...
        temp_audio_path = "temp_response_audio.mp3"
        tts.save(temp_audio_path)

        # Play the audio from the temporary location
        ...
        st.write("Please upload a valid report or prescription.")
